...
export default function AdminPage() {
  ...
  const reposWithGithub = apps.filter(a => a.repo).map(a => a.repo);
  const githubData = useAllGitHubData(reposWithGithub);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" key={key}>
        ...
        <div className="space-y-4">
          {apps.map((app) => {
            const gh = app.repo ? githubData[app.repo] : null;
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {app.repo && (
                      <a
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-emerald-300 dark:hover:border-emerald-700 transition-all flex-shrink-0"
                      >
                  ...
                  {!app.repo && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                      <p className="text-sm text-gray-400 dark:text-gray-500">
                        {locale === "ru"
                          ? "Нет публичного репозитория на GitHub"
